...
6. **Skill nuove o aggiornate**: controlla come sono installate le skill in questa sessione. Se l'utente ha il **plugin** (skill con prefisso `penalista-italia:`), le skill si aggiornano col marketplace: guidalo passo passo — **Impostazioni → Plugin → Sfoglia** (si apre la Directory) → **Plugin → Personale** → tre puntini **···** sul chip `penalista-italia` → **"Verifica aggiornamenti"** (il "Commit sincronizzato" deve cambiare), poi riavvio e conversazione nuova; verifica finale con `/penalista-italia:versione`, che confronta da solo la versione installata con GitHub. Se il client dice "aggiornato" ma resta fermo a una versione vecchia (blocco silenzioso noto della sincronizzazione), fai eseguire la procedura di sblocco: **··· → Rimuovi** sul chip del marketplace, poi **+** → ri-aggiungere `Synthos-Logic/penalista-italia`, reinstallare il plugin, riavviare — fascicoli e Knowledge Base non vengono toccati. Solo se l'utente usa le skill standalone (senza plugin): identifica dal CHANGELOG quali skill sono nuove o modificate, crea i pacchetti `.skill` (procedura in `INSTALLAZIONE_ASSISTITA.md`) e presentali con il pulsante "Salva skill", avvertendo di eliminare prima le versioni vecchie da Personalizza → Skills.
